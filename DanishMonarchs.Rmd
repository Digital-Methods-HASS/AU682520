...
# glimpse(kings2), på den måde ser vi at der er 22 kolonner
...
kings <- read_csv("data/danskekonger.csv")
...
```{r}
kings <- kings %>%
  mutate((reign_duration = kings$`Regerings slut` - kings$`Regerings start`))
...
```{r}
mean(kings$`Regerings slut`- kings$`Regerings start`, na.rm = TRUE)

kings%>%
  summarize(mean(kings$`Regerings slut`- kings$`Regerings start`, na.rm = TRUE))
...
kings_abv_avg <- kings %>% 
  filter(kings$`Regerings slut` - kings$`Regerings start` >=18.98276)
...
```{r}
kings %>% 
  slice_max( kings$`Regerings slut`- kings$`Regerings start`, n = 3)
...
```{r}
kings6 <- kings %>%
  mutate(midyear = kings$`Regerings start`+ kings$`Regerings slut`/2)

kings6 %>%
  
  ggplot(aes( x = midyear, y = kings$`Regerings slut`- `Regerings start`))+ 
  geom_smooth()+ 
  geom_point()+ 
  theme_bw()
