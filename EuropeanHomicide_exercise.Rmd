...
danish_kings <- read_csv("data/danskekonger.csv")

head(danish_kings)

midyear = danish_kings$Regerings.slut - (danish_kings$Regerings.slut - danish_kings$Regerings.start)/2

danish_kings <- danish_kings %>%
  mutate(midyear = `Regerings slut` - ( danish_kings$`Regerings slut` - danish_kings$`Regerings start` )/2 )

ggplot(danish_kings, aes(x = midyear, y = danish_kings$`Regerings slut` - danish_kings$`Regerings start`)) +
  geom_point(color = "blue", size = 3) +
  geom_smooth(method = "loess", color = "red", se = FALSE) +
  labs(title = "Varighed af Danske kongers regering over tid",
       x = "Midtår af regeringstid",
       y = "Varighed af regeringstid (år)") +
         theme_minimal()
```
